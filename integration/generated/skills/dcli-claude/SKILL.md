---
name: dcli-claude
description: Delegate bounded work to another backend. Use when cross-backend wrapper job guarantees are needed.
---
...
| Effort/reasoning | `--effort <level>`; enum: low, medium, high, xhigh, max |

## Flags specific to this backend

- `--effort <level>` — reasoning effort. `--variant` is rejected. A value outside the enum is rejected with exit `2` before a job is created.
