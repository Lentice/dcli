...
// ===========================================================================
{
  ...
  // cmd.exe spawns successfully and exits on its own; it is not codex, but
  // Start() only has to launch the child and wire its streams.
...
// ===========================================================================
// 2. A genuinely failing spawn still cleans up the temp dir and rethrows the
//    real spawn error — not a ReferenceError masking it
// ===========================================================================
{
  const before = countTempDirs();
  const { CodexAdapter } = require('../../../adapters/codex/adapter');
  ...
  const savedPath = process.env.CODEX_PATH;
  // A .cmd path cannot be spawned by Node at all (EINVAL since 18.20/20.12),
  // and buildCmdInvocation hands it to cmd.exe — so force a hard sync failure
  // by pointing ComSpec-independent resolution at a directory.
  ...
  if (error) {
    ...
    // Spawn succeeded on this host; that is acceptable — the assertion that
    // matters is that no ReferenceError was raised.
    try { adapter._childProcess.kill(); } catch {}
  }
